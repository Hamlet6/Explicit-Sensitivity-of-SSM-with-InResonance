function [W_0i, R_0i,H] = cohomological_solution(obj, i,  W_0, R_0,H,data)
...
switch obj.Options.notation
    case 'tensor'
        ...
        parfor l = 1:m^i
            ...
            W_0i(:,l) = solveinveq(C_l,b_l,solver);
        end
    ...
    case 'multiindex'
        ...
        switch nl_data.mode
            case 'IntrusiveF' 
                ...
                for n = 2:min(k,nl_data.order) 
                    if ~isempty(NL(n)) && ~isempty(NL(n).coeffs) && ~isempty(NL(n).ind)
                        Fn  = Fn + NL(n).coeffs* fnl_intrusive(NL(n).ind.',K,sys);
                    end
                end
                sys = rmfield(sys,'H');
        ...
        if strcmp(obj.System.Options.COMPtype,'first')
