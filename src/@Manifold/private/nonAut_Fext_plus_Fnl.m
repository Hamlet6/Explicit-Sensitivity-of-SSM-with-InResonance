...
for n = 2:k+1
    ...
    if  n <= data.Fext_ord(i) && ~isempty(FEXT.data(i).F_n_k(n).coeffs)
        ...
        Force(i).val   = Force(i).val + F_coeff * compute_pi(F_ind,K, data);
    ...
    switch data.mode
        ...
        case 'IntrusiveF'
            if n <= data.nl_ord && ~isempty(NL(n)) && ~isempty(NL(n).coeffs)
                
                sigma = dfnl_intrusive(NL(n).ind.',W1,k,data,nKappas);
                for i = 1:nKappas
                    DF(i).val  = DF(i).val + NL(n).coeffs* sigma(i).val;
                end
            end
        case 'NonIntrusiveF'
