...
% FNL_INTRUSIVE This function computes the nonlinear contributions to the invariance
%
...
% matrices Multi index arrays n_mat and k_mat need to contain multi indices
% of same order |n| and |k| resepectively. The pi's are the forcing contribution
% coefficients as given in $\textit{Computing SSMs using multi indices - autonomous}$
% which can be found in the Documentation.
%
% [pis] = FNL_INTRUSIVE(n_mat,k_mat, field)
%
% n_mat:    multi indices of function which is being composed with
%           parametrisation
...
%
% pis:      composed forcing contributions for the combination of input
%           multi-indices
%
...
switch data.ordering
    case 'revlex'
        ...
        pis_pos = pi_nk(n_mat(:,n_pos_idx),k_mat(:,k_pos_idx), data);
       
    case 'conjugate'
        %Needs all multi-indices in n_mat and k_mat to have same order
        pis_pos = pi_nk_conj(n_mat(:,n_pos_idx),k_mat(:,k_pos_idx),data);
end
...
function [pis] = pi_nk(n_mat,K,field)
...
function [pis] = pi_nk_conj(F_multi,K,field)
...
% $\texttt{combnos}$ contains the amount of combinations that exist for every
% tuple $(\texttt{f,i})$.
[h, combnos,sz_un,sz_un_ic] = multi_nsumk(sz_pos,K,'remove_trivial_idcs');
...
for f = 1:z_k
    ...
    for i = 1:size(sz_un,2)
        ...
        % Index of multi-idcs in the unique multi-idcs
        h_i_idx_tmp = multi_index_2_ordering(h_i_un.',field.ordering,revlex2conj);
