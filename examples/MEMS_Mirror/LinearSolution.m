%% MYMEMS
%clearvars
% We compute SSM compute for a geometrically nonlinear Timoshenko Beam

clc
% Generate model
order = 3;
forcingdirection = 'z';

[M,C,K,fnltens,fext,outdof] = build_model(forcingdirection);
n = length(M);


mode = 1;
masterModes = [2*mode-1,2*mode]; 
epsilon = 5e-9; % 


% Dynamical system setup
DS = DynamicalSystem();
set(DS,'order',2)
set(DS,'M',M,'C',C,'K',K,'fnl',fnltens);
set(DS.Options,'Emax',4,'Nmax',10,'notation','multiindex')

% forcing
kappas = [-1; 1];
coeffs = [fext fext]/2;
DS.add_forcing(coeffs, kappas, epsilon);

% modal analysis
[V,D,W] = DS.linear_spectral_analysis();

%% 
% *Choose Master subspace*
S = SSM(DS);
set(S.Options, 'reltol', 0.1,'notation','multiindex')
S.choose_E(masterModes);

omega0 = imag(S.E.spectrum(1));
omegaRange = omega0*[0.9 1.1] ;

%% Nonlinear results
set(S.Options, 'reltol', 0.5,'IRtol',0.05,'notation', 'multiindex','contribNonAuto',false)
set(S.FRCOptions, 'nt', 2^8, 'nRho', 200, 'nPar', 200, 'nPsi', 100, 'rhoScale',0.1)
set(S.FRCOptions, 'outdof',outdof)
set(S.FRCOptions, 'method','level set') 
 
% extract forced response curve
FRC = S.extract_FRC('freq',omegaRange,order);

%% Linear solution
set(DS.Options,'outDOF',outdof)
Omegas = linspace(omegaRange(1),omegaRange(2),100);
[response,Znorm,Aout] = linear_response(DS,Omegas);


%% Compare the two solutions

plot(Omegas,Aout,'-','DisplayName','Linear');


